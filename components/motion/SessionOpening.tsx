...
${JSON.stringify(sources)}.forEach(function(s){var e=document.createElement('source');e.src=s[0];e.type=s[1];v.appendChild(e);});
v.load();var p=v.play();if(p&&p.catch){p.catch(fin);}v.addEventListener('ended',fin);setTimeout(fin,${tokens.duration.opening + 300});var b=w.querySelector('[data-skip]');if(b){b.addEventListener('click',fin);}document.addEventListener('keydown',function(e){if(e.key==='Escape'){fin();}});})();`;
