...
function Detalhe({ id, sizes, className }: { id: string; sizes: string; className?: string }) {
  if (!hasImage(id)) {
    return <div className={["detalhe-placeholder", className].filter(Boolean).join(" ")} aria-hidden="true" />;
  }
  return <MediaImage id={id} sizes={sizes} className={className} imgClassName="h-full w-full object-cover" />;
}
    ...
    <>
      ...
      <section className="hero" data-hero>
        ...
        <div className="container-site hero-inner">
          <div className="max-w-3xl">
            ...
            <WordTitle text={pagina.seo.h1} className="text-display" delay={motionTokens.duration.short} />
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <TrackedLink href={reservasUrl} event="reservar_click" params={{ origem: "hero" }} className="btn btn-primary">
                {hero.cta_primario}
                <Arrow />
              </TrackedLink>
      ...
      <Section id="para-quem" kicker={jeitos.kicker} title={jeitos.titulo} tone="branco">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {cards.map((c, i) => (
            <Reveal key={c.titulo} as="article" delay={i * motionTokens.stagger.item} className="card-zoom rounded-2xl bg-bege">
              <div className="aspect-[4/3] overflow-hidden rounded-t-2xl">
                <Detalhe id={c.image} sizes="(min-width: 48rem) 33vw, 100vw" className="h-full w-full" />
              </div>
              <div className="p-6">
      ...
      <Section id="o-nome" kicker={nome.kicker} title={nome.titulo}>
        <div className="mt-8 grid items-center gap-10 lg:grid-cols-[1fr_auto]">
          ...
          <StrokeSymbol className="h-40 w-40 justify-self-center text-cafe lg:h-64 lg:w-64" title="Símbolo do Kaluanã" />
        </div>
      </Section>
      ...
      <Section id="restaurante" kicker={restaurante.kicker} title={restaurante.titulo}>
        <div className="mt-8 grid items-center gap-10 lg:grid-cols-2">
          <Reveal className="measure">
            {restaurante.texto ? <CopyParagraphs text={restaurante.texto} className="text-xl" /> : null}
          ...
          <Reveal variant="mask" className="card-zoom aspect-[4/3] overflow-hidden rounded-2xl">
            <Detalhe id={detalhes.restaurante} sizes="(min-width: 64rem) 50vw, 100vw" className="h-full w-full" />
          </Reveal>
        </div>
      </Section>
      ...
      <Section id="ji-parana" kicker={jiparana.kicker} title={jiparana.titulo} tone="branco">
        <div className="mt-8 grid items-center gap-10 lg:grid-cols-2">
          <Reveal className="measure">
            ...
            <TrackedLink href="/ji-parana" event="mapa_click" params={{ origem: "home" }} className="btn btn-secondary mt-8">
      ...
      <section id="chamada-final" className="relative overflow-hidden bg-cafe text-bege">
        ...
        <div className="container-site section-y relative text-center">
          <Reveal className="mx-auto max-w-3xl">
            ...
            {final.texto ? <CopyParagraphs text={final.texto} className="mt-4 text-xl text-bege/90" /> : null}
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <TrackedLink href={reservasUrl} event="reservar_click" params={{ origem: "chamada_final" }} className="btn btn-inverse">
                {final.cta_primario}
                <Arrow />
              </TrackedLink>
