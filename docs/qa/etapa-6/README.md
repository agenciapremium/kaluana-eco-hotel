...
| Acessibilidade, teclado    | 42 percursos e 12 roteiros por fase, todos aprovados       | `a11y-pre.json`, `a11y-full.json`                       |
...
| Contraste sobre foto       | 93 textos por fase, nenhum abaixo do mínimo (menor 5,68:1) | `contraste-cenas-pre.json`, `contraste-cenas-full.json` |
...
Na fase `full`, que tem o menu completo e o número do quarto nos cards e na barra de hóspede, o resultado final é o mesmo: 213 medições do axe sem violação, 42 percursos com 1.568 paradas, os 12 roteiros e as 21 árvores aprovados. As rodadas dessa fase acharam dois defeitos que só existem nela, as cores de andar abaixo de 4,5:1 e o Tab saindo do menu aberto do celular; os dois foram corrigidos e medidos de novo. As duas fases foram medidas depois do véu atrás do texto.

**Contraste de texto sobre foto.** Quando o texto fica sobre imagem, vídeo ou gradiente, o axe marca o caso como incompleto e não reprova. `npm run qa:contraste` mede esses casos nas cenas. Ele fotografa a caixa de cada texto claro na Home, no hub do Universo, nos hubs de Peixes, Árvores, Aves e Guardiões e em quatro páginas de elemento, no desktop e no celular, e compara a cor do texto com a parte mais clara do fundo, o percentil 95 dos pixels (decisão 112). Na fase `full`, antes da correção, 19 de 93 textos ficavam abaixo do mínimo: o texto do hero e dos painéis da Home, os ordinais e os nomes dos blocos do hub do Universo, o "2º andar" do hub de Peixes e o kicker do hero do pirarucu e da samaúma. Com o véu local atrás de cada bloco de texto (decisão 111), **nenhum texto fica abaixo do mínimo nas duas fases**. O menor contraste é 5,68:1, no nome "Coruja", num painel da Home.

Achados corrigidos (decisões 95 a 104, 107, 109 e 111):
