...
| **Configurar e publicar as tags no contêiner `GTM-KTMNR36W`.** O site carrega o GTM depois do consentimento desde 21/09/2026, mas a versão publicada do contêiner (versão 2) não tem nenhuma tag: nada é medido. Falta a tag do GA4, o Pixel e a CAPI da Meta, e um gatilho de evento personalizado para cada um dos 9 eventos da camada de dados (`reservar_click`, `lead_pre_inauguracao`, `lead_evento`, `lead_corporativo`, `qr_scan`, `whatsapp_click`, `mapa_click`, `cardapio_open`, `audio_play`), que chegam com os parâmetros `origem`, `perfil`, `andar`, `elemento` e `uh` | Premium           | painel do GTM                       |
